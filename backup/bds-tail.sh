...
conectar() {
  local user="$1"
  sleep infinity |
    sshpass -p "$RCON_PASSWORD" ssh -tt -q \
...
# Se reconecta sola: si BDS se reinicia, la sesion muere y sin esto el volcado
# se perderia en silencio, que es justo el fallo que este script viene a evitar.
#
# El usuario que acepta mc-server-runner no esta documentado. Se prueban varios,
# y en cuanto uno aguanta una sesion se RECUERDA: rotar usuarios en cada
# reconexion llenaria los logs de intentos fallidos.
usuario_ok=""
while true; do
  for u in ${usuario_ok:-$BEDROCK_SSH_USERS}; do
    log "conectando como '$u'..."
    inicio=$SECONDS
    # tr quita los  del pseudo-tty, que si no parten las lineas en los logs.
    conectar "$u" | tr -d '' | sed -u 's/^/[bds] /'
    duro=$(( SECONDS - inicio ))
